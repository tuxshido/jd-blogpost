...
def search():
    ...
    if searchby == "title":
        posts = Blogpost.query.filter(Blogpost.title.contains(
            searchword)).order_by(Blogpost.date_posted.desc()).all()
    else:
        posts = Blogpost.query.filter(Blogpost.tag.contains(
            searchword)).order_by(Blogpost.date_posted.desc()).all()
    if "google_id" not in session:
        return render_template('index.html', posts=posts, login=False, name=None)
    else:
...
def addpost():
    ...
    if (title != "" and tag != "" and content != ""):
        post = Blogpost(title=title, tag=tag, author=author,
